...
function Message({ selectedMenu, setSelectedMenu }) {
  ...
  useEffect(() => {
    ...
    if (msgCount===0) {
      toast.warning('Please recharge as your current plan has expired.');
    }
    loadUserData();
  }, [globalFilter,msgCount]);
  ...
  const sendMessage = async (e) => {
    if ((tag !== "" && selectedData.length !== 0) && (videoid != "" || message != "" || filepath != "") && (selectedData.length <= msgCount) ) {
      ...
      {
        toast.warning("Plan expired. Please recharge to send further messages.")
      }
